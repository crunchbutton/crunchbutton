...
class PriorityNotificationTest extends PHPUnit_Framework_TestCase
{
    ...
    public static function checkDescendingOrder($array) {
        $check = true;
        $count = $array->count();
        for ($i=0; $i <= $count; $i++) {
            $a = $array->get($i);
...
//        $communityId3 = $community3->id_community;
    ...
    public function testRegister1() {
        ...
        Crunchbutton_Admin_Notification_Log::register($o1->id_order);
        $amls = Crunchbutton_Admin_Notification_Log::q('select * from admin_notification_log where id_order = ?', [$o1->id_order]);
        $numAttemptsNoAdmin = Crunchbutton_Admin_Notification_Log::attemptsWithNoAdmin($o1->id_order);
        $numAttemptsAdmin = Crunchbutton_Admin_Notification_Log::attemptsWithAdminAndCutoff($o1->id_order, $this->driver1->id_admin);
        $sortedAttemptsWithAdmin = Crunchbutton_Admin_Notification_Log::sortedAttemptsWithAdmin($o1->id_order, $this->driver1->id_admin);
        $o1->delete();
        $n1->delete();
        $amls->delete();
        $this->assertEquals($amls->count(), 1);
        $this->assertEquals($numAttemptsNoAdmin, 1);
        $this->assertEquals($numAttemptsAdmin, 0);
        $this->assertEquals($sortedAttemptsWithAdmin->count(), 0);
    }

    public function testRegister2a() {
        ...
        $n1->delete();
        $amls->delete();
        $this->assertEquals($amls->count(), 2);
        $this->assertEquals($numAttemptsNoAdmin, 2);
        $this->assertEquals($numAttemptsAdmin, 0);
        $this->assertEquals($sortedAttemptsWithAdmin->count(), 0);
    ...
    public function testRegisterWithAdmin1a() {
    ...
    public function testRegisterWithAdmin2a() {
        ...
        $this->assertEquals($amls2->count(), 1);
        $this->assertEquals($amls3->count(), 0);
        $this->assertEquals($numAttemptsNoAdmin, 0);
        $this->assertEquals($numAttemptsAdmin1, 0);
        $this->assertEquals($numAttemptsAdmin2, 0);
    ...
    public function testRegisterWithAdmin3b() {
        ...
        $this->assertEquals($numAttemptsNoAdmin, 0);
        $this->assertEquals($numAttemptsAdmin1, 0);
        $this->assertEquals($numAttemptsAdmin2, 1);
        $this->assertEquals($sortedAttemptsWithAdmin1->count(), 0);
        $this->assertEquals($sortedAttemptsWithAdmin2->count(), 2);
        $this->assertEquals($numAttemptsCheck, 1);
        $this->assertEquals($hasUnexpired1, false);
    ...
    public function testRegisterWithAdmin3c() {
    ...
    public function testOrderedOrderPriority1() {
        ...
        $op1 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver1,
            $useDateString, Crunchbutton_Order_Priority::PRIORITY_LOW, 240, $laterDateString);
        $op1->save();
        $op2 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver2,
            $useDateString, Crunchbutton_Order_Priority::PRIORITY_HIGH, 0, $laterDateString);
        $op2->save();
        $op3 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver3,
            $useDateString, Crunchbutton_Order_Priority::PRIORITY_LOW, 240, $laterDateString);
        $op3->save();

        $ops = [$op1, $op2, $op3];

        $opsDb = Crunchbutton_Order_Priority::getOrderedOrderPriorities($o1->id_order);
        ...
        $o1->delete();
        $counter = 0;
        $this->assertEquals($opsDb->count(), 3);
        foreach ($opsDb as $op) {
            if ($op->id_admin == $this->driver1->id_admin) {
                if ($counter == 0 || $counter == 1) {
                    ...
                    $driver1Correct = false;
                }
                $this->assertEquals($driver1Correct, true);

            }
            else if ($op->id_admin == $this->driver3->id_admin) {
                if ($counter == 0 || $counter == 1) {
                    $driver3Correct = true;
                }
                else{
                    $driver3Correct = false;
                }
                $this->assertEquals($driver3Correct, true);

            }
            else if ($op->id_admin == $this->driver2->id_admin) {
                if ($counter == 2) {
                    $driver2Correct = true;
                }
                else{
                    $driver2Correct = false;
                }
                $this->assertEquals($driver2Correct, true);
    ...
    public function testGetDeliveryLogisticsByOrder1() {
        ...
        $o1->delete();
        $olt = Crunchbutton_Order_Priority::getDeliveryLogisticsByOrder($o1->id_order);
        $this->assertEquals(0, $olt);

    }

    public function testGetDeliveryLogisticsByOrder2() {
        ...
        $op1 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver1,
            $useDateString, Crunchbutton_Order_Priority::PRIORITY_LOW, 240, $laterDateString);
        $op1->priority_algo_version = 50;
        $op1->save();
        $op2 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver2,
            $useDateString, Crunchbutton_Order_Priority::PRIORITY_HIGH, 0, $laterDateString);
        $op2->priority_algo_version = 50;
        $op2->save();
        $op3 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver3,
            $useDateString, Crunchbutton_Order_Priority::PRIORITY_LOW, 240, $laterDateString);
        $op3->priority_algo_version = 50;
        $op3->save();
        ...
        $o1->delete();
        $this->assertEquals(Crunchbutton_Order_Logistics::LOGISTICS_SIMPLE, $olt);

    }

    public function testGetDeliveryLogisticsByOrder3() {
        ...
        $o1->delete();
        $this->assertEquals(Crunchbutton_Order_Logistics::LOGISTICS_COMPLEX, $olt);

    }

    public function testGetDeliveryLogisticsByOrder4() {

    $complexVersion = Crunchbutton_Order_Priority::COMPLEX_CUTOFF + 1000;
    ...
    $op1 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver1,
        $useDateString, Crunchbutton_Order_Priority::PRIORITY_LOW, 240, $laterDateString);
    $op1->priority_algo_version = $complexVersion;
    $op1->save();
    $op2 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver2,
        $useDateString, Crunchbutton_Order_Priority::PRIORITY_HIGH, 0, $laterDateString);
    $op2->priority_algo_version = $complexVersion;
    $op2->save();
    $op3 = $this->defaultOrderPriority($o1, $this->restaurant1, $this->driver3,
        $useDateString, Crunchbutton_Order_Priority::PRIORITY_LOW, 240, $laterDateString);
    $op3->priority_algo_version = $complexVersion;
    $op3->save();
    ...
    public function testCheckForNoRepsNotifications() {
...
//    }
    ...
    public function createDefaultAdminNotificationLogWithAdmin($id_order, $id_admin, $description, $date) {
        return new Crunchbutton_Admin_Notification_Log(
            [
                'id_order' => $id_order,
                'id_admin' =>$id_admin,
