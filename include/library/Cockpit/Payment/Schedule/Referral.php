...
class Cockpit_Payment_Schedule_Referral extends Cana_Table {
	...
	public function checkReferralWasPaidDriver( $id_referral ){
		$query = 'SELECT * FROM payment_schedule_referral INNER JOIN payment_schedule ON payment_schedule.id_payment_schedule = payment_schedule_referral.id_payment_schedule WHERE id_referral = ? AND payment_schedule.payment_status != ? LIMIT 1';
