...
class Cockpit_Payment_Schedule_Shift extends Cana_Table {
	...
	public static function checkShiftWasPaidDriver( $id_admin_shift_assign ){
		$query = 'SELECT * FROM payment_schedule_shift INNER JOIN payment_schedule ON payment_schedule.id_payment_schedule = payment_schedule_shift.id_payment_schedule WHERE id_admin_shift_assign = ? AND payment_schedule.payment_status != ? LIMIT 1';
