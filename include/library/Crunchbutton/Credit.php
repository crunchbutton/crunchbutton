...
class Crunchbutton_Credit extends Cana_Table
{
	...
	public function creditByUser( $id_user ) {
		return Crunchbutton_Credit::q('SELECT * FROM credit WHERE type = "' . self::TYPE_CREDIT . '" AND id_user="'.$id_user.'" AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '"');
	}

	public function debitByUser( $id_user ) {
		return Crunchbutton_Credit::q('SELECT * FROM credit WHERE type = "' . self::TYPE_DEBIT . '" AND id_user="'.$id_user.'" AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '"');
	}
	...
	public function creditByUserValidToAnyRestaurant( $id_user ){
		...
		$query = 'SELECT SUM(`value`) as debit FROM ( SELECT SUM(`value`) as `value` FROM credit WHERE type = "' . self::TYPE_CREDIT . '" AND id_user = '.$id_user.' AND id_restaurant IS NULL  AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '" UNION SELECT SUM( value ) * -1  FROM credit WHERE id_credit_debited_from IN ( SELECT id_credit FROM credit WHERE type = "' . self::TYPE_CREDIT . '" AND id_user = '.$id_user.'  AND id_restaurant IS NULL  AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '" ) ) debit';
		$row = Cana::db()->get( $query );
		if( $row->_items && $row->_items[0] ){
	...
	public function totalDebitedFromCredit(){
		$query = 'SELECT SUM(`value`) as `debited` FROM credit WHERE type = "' . self::TYPE_DEBIT . '"  AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '" AND id_credit_debited_from = ' . $this->id_credit;
		$row = Cana::db()->get( $query );
		if( $row->_items && $row->_items[0] ){
	...
	public static function find($search = []) {
		...
		$query = 'SELECT `credit`.* FROM `credit` LEFT JOIN restaurant USING(id_restaurant) WHERE id_credit IS NOT NULL AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '"';
	...
	public function creditByOrder( $id_order ) {
		return Crunchbutton_Credit::q('SELECT * FROM credit WHERE id_order="'.$id_order.'"  AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '"');
	}

	public function creditByOrderPaidBy( $id_order, $paid_by ) {
		$query = 'SELECT SUM(`value`) as credit FROM credit WHERE id_order = ' . $id_order . ' AND type = "' . Crunchbutton_Credit::TYPE_DEBIT . '" AND paid_by = "' . $paid_by . '"  AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '"';
		$row = Cana::db()->get( $query );
		if( $row->_items && $row->_items[0] ){
			if( $row->_items[0]->credit ){
	...
	public function debitHistory(){
		return Crunchbutton_Credit::q('SELECT * FROM credit WHERE id_credit_debited_from="'.$this->id_credit.'"  AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '"');
	}
	...
	public function creditSpent(){
		$query = 'SELECT SUM( value ) as spent FROM credit c WHERE type = "' . Crunchbutton_Credit::TYPE_DEBIT . '" AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '" AND id_credit_debited_from = ' . $this->id_credit;
		$row = Cana::db()->get( $query );
		if( $row->_items && $row->_items[0] ){
				$row = $row->_items[0];
	...
	public function creditsAvailableByUserRestaurant( $id_user, $id_restaurant ){
		...
		$credits = Crunchbutton_Credit::q( 'SELECT * FROM credit WHERE type = "' . self::TYPE_CREDIT . '" AND id_user="'.$id_user.'" AND credit_type = "' . Crunchbutton_Credit::CREDIT_TYPE_CASH . '" AND ( id_restaurant = '.$id_restaurant.' OR id_restaurant IS NULL )' );
		if( $credits->count() > 0 ){
		...
		$query = "SELECT (credit - COALESCE( debits.debit, 0)) AS credit_left,
							...
							FROM
								(SELECT SUM(c.value) AS credit,
								 ...
								 WHERE c.type = 'CREDIT' AND credit_type = '" . Crunchbutton_Credit::CREDIT_TYPE_CASH . "'
							...
							LEFT JOIN
								(SELECT SUM(c.value) AS debit,
								 ...
								 FROM credit c
								 INNER JOIN user u ON u.id_user = c.id_user
								 WHERE c.type = 'DEBIT' AND credit_type = '" . Crunchbutton_Credit::CREDIT_TYPE_CASH . "'
		...
		$query = "SELECT credits.credit,
							...
							FROM
								(SELECT SUM(value) AS credit
								 FROM credit c
								 INNER JOIN user u ON u.id_user = c.id_user
								 WHERE TYPE = 'CREDIT' AND credit_type = '" . Crunchbutton_Credit::CREDIT_TYPE_CASH . "'
									 AND u.id_user = '{$id_user}') credits,
								(SELECT SUM(value) AS debit
								 FROM credit c
								 INNER JOIN user u ON u.id_user = c.id_user
								 WHERE TYPE = 'DEBIT' AND credit_type = '" . Crunchbutton_Credit::CREDIT_TYPE_CASH . "'
