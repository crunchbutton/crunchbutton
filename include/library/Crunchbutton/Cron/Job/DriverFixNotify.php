...
//		';
		...
		left join restaurant using (id_restaurant)
		left join admin_notification_log using (id_order)
		left join community as c on `order`.id_community = c.id_community
		where restaurant.delivery_service=1 and restaurant.active=true and admin_notification_log.id_admin_notification_log is null
		and `order`.date > date_sub(now(), interval 10 minute) and `order`.date < date_sub(now(), interval 1 minute) and
		(c.id_community is null or (c.id_community is not null and c.delivery_logistics is null))
		order by `order`.id_order desc
		';
		$orders = Order::q($q);
