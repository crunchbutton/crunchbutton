...
NGApp.directive( 'driverDocsUpload', function ( $fileUploader, $rootScope, $timeout ) {
	return {
		restrict: 'AE',
		replace: true,
		compile: function( elem, attrs ) {
				var template = '<div>' +
													'<button class="ladda-button upload" data-color="mint" data-style="expand-right" data-size="l"><span class="ladda-label"><b>UPLOAD</b></span></button>' +
													'<br/>' +
				...
				var link = function ( scope, elem, attrs ) {
					// need to wrap the all the code at the timeout
					// because it take some milisecs to build the dom
					// if we remove the timeout Ladda will not work correctly
					$timeout( function(){

						// elements that will be used button and file input
						var button = elem.find( '.upload'  );
						var file = elem.find( '.file' );
						...
						button.bind( 'click', function(){
							// fake a click at the file input
							file.click();
						} );

						file.bind( 'change', function(a,b,c){
							// start ladda
							l.start();
							// wait some milisecs and start upload
							setTimeout( function(){ uploader.uploadAll(); }, 100 );
						} );
						...
						uploader.bind( 'success', function( event, xhr, item, response ) {
							...
							uploader.clearQueue();
							// stop ladda
							setTimeout( function(){ l.stop(); }, 100 );
						});
						// broadcast error :(
