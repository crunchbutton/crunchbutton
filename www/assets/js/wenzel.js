...
//	App.page.restaurant('alpha-delta-pizza');
};
...
App.page.restaurant = function(id) {
	...
	App.cache('Restaurant', id, function() {
		...
		var dish = $('<li><a href="javascript:;" data-id_dish="' + App.restaurant.top().id_dish + '"><span class="dish-name">' + App.restaurant.top().name + '</span><span class="dish-price">($' + App.restaurant.top().price + ')</span></a></li>');
			
		$('.restaurant-items').append('<div class="content-item-name content-item-main-name"><h1>Add to your order</h1></div>', dish);

		list = $('<ul class="resturant-dishes resturant-dish-container"></ul>');
			
			$('.restaurant-items').append('<div class="restaurant-item-title">Add another</div>',
				list
			);
			list.append(dish);
...
App.drawPay = function(restaurant) {
	...
	$('.main-content').append(
		...
		'<div class="button-bottom-wrapper" data-role="footer" data-position="fixed"><button class="button-submitorder-form button-bottom"><div>Wenzel Me</div></button></div>'
	);
